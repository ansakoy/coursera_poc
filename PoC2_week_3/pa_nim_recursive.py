...
TEMPLATE: http://www.codeskulptor.org/#poc_nim_recursive_template.py
SOLUTION: http://www.codeskulptor.org/#poc_nim_recursive_solution.py
"""
...
def evaluate_position(current_num):
    ...
    for move in xrange(1, min(MAX_REMOVE + 1, current_num + 1)):
        if evaluate_position(current_num - move) == 'lost':
            return 'won'
    return 'lost'
...
def evaluate_memo_position(current_num, memo_dict):
    ...
    for move in xrange(1, min(MAX_REMOVE + 1, current_num + 1)):
        if current_num in memo_dict:
            ...
            memo_dict[current_num] = evaluate_memo_position(current_num - move, memo_dict)
            if memo_dict[current_num] == 'lost':
                return 'won'
    return memo_dict[current_num]
