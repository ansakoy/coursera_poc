# TEMPLATE: http://www.codeskulptor.org/#poc_dice_game_template.py
...
def max_repeats(seq):
    ...
    in a sequence
    """
    return max([seq.count(element) for element in seq])
...
def compute_expected_value(outcomes):
    ...
    """
    all_variants = gen_all_sequences(outcomes, 3)
    max_rep = [max_repeats(sequence) for sequence in all_variants]
    expected = 0
    for result in max_rep:
        if result == 2:
            expected += 10 / 216.0
        elif result == 3:
            expected += 200 / 216.0
